...
document.getElementById("beritaAcara").addEventListener("submit", async function (e) {
  ...
  try {
    const response = await fetch("http://localhost:5000/api/serah-terima/add", {
      method: "POST",
      body: formData,
    });
...
document.getElementById("editForm").addEventListener("submit", async function (e) {
  ...
  try {
    const response = await fetch("http://localhost:5000/api/serah-terima/edit/" + id, {
      method: "PUT",
      body: formData,
    });
...
function confirmDeleteBerita() {
  ...
  fetch(`http://localhost:5000/api/serah-terima/delete/${deleteId}`, {
    method: "DELETE",
  })
    .then((response) => {
      if (response.ok) {
...
async function loadBerita() {
  try {
    const response = await fetch("http://localhost:5000/api/serah-terima");
