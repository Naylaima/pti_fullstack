...
router.post("/add", (req, res) => {
  ...
  try {
    const query =
      "INSERT INTO serah_terima (nomor, nama, from_unit, to_unit, keterangan, nama_file) VALUES (?, ?, ?, ?, ?, ?)";
...
router.get("/", (req, res) => {
  const query = "SELECT * FROM serah_terima";
...
router.put("/edit/:id", (req, res) => {
  ...
  if (file) {
    ...
    // delete file lama
    query = "SELECT nama_file FROM serah_terima WHERE id = ?";
...
router.delete("/delete/:id", (req, res) => {
  ...
  try {
    query = "DELETE FROM serah_terima WHERE id = ?";
    db.query(query, [id], (err, result) => {
      if (err) return res.status(500).send(err.message);
